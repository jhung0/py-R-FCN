...
class try1(imdb):
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'data')
    ...
    def _load_try1_annotation(self, index):
        """
        Load image and bounding boxes info from XML file in the try1 format.
        """
	...
	print index
	    ...
	    seg_areas[ix] = (float(x2) - float(x1) + 1) * (float(y2) - float(y1) + 1)
    ...
    def _get_try1_results_file_template(self):
        # VOCdevkit/results/VOC2007/Main/<comp_id>_det_test_aeroplane.txt
        filename = self._get_comp_id() + '_det_' + self._image_set + '_{:s}.txt'
        path = os.path.join(self._devkit_path, 'results', self.name, str(os.getpid())) + '_det_' + self._image_set + '_{:s}.txt'
	return path
    ...
    def _write_try1_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} try1 results file'.format(cls)
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_try1_results_file(all_boxes)
